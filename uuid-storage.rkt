...
(provide current-uuid-store
         current-uuid-store-custodian
         make-uuid-store
         uuid-store-contains?
         uuid-store-read
         uuid-store-write!
...
(: current-uuid-store (Parameterof (Option Path)))
(define current-uuid-store
  (make-parameter #f))

(: current-uuid-store-custodian (Parameterof Custodian))
(define current-uuid-store-custodian
  (make-parameter (current-custodian)))
...
(struct: (A) uuid-box ([uuid : String]
                       ...
                       [content : (Boxof (Option (Custodian-Boxof A)))]))
  ...
  (let ([uuid (uuid-generate)])
    (begin
      ...
      (uuid-box uuid
                ...
                (box (make-optional (make-custodian-box (current-uuid-store-custodian) value)))))))
...
(define (fetch-uuid-box store uuid)
  (if (uuid-store-contains? store uuid)
      (uuid-box uuid (bake-uuid-store store) (box (ann #f (Option (Custodian-Boxof A)))))
      #f))
...
(define (uuid-unbox ref-box)
  (let ([outer (unbox (uuid-box-content ref-box))])
    (if outer ; this is sort of like a backwards cond, that would be a fun macro
        (let ([inner (custodian-box-value outer)])
          (if inner
              inner
...
(define (uuid-unbox/refresh ref-box)
  (let ([value ((baked-uuid-store-read (uuid-box-store ref-box)) (uuid-box-uuid ref-box))])
    (set-box! (uuid-box-content ref-box)
              (make-custodian-box (current-uuid-store-custodian) value))
    value))
...
(define (set-uuid-box! ref-box value)
  ...
  (set-box! (uuid-box-content ref-box) (make-custodian-box (current-uuid-store-custodian) value)))
